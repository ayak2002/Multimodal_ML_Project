...
class SingleCellDataset(Dataset):
    ...
    def __init__(
        ...
        is_train: bool,
        ssl_flag: bool,
        include_labels: bool = False, #adding this to make cleaner must pass it in dataset_utils if you want labels at eval time 4/6
        ...
        print("DEBUG: CSV path being loaded =", csv_path)
        ...
        print("[DEBUG] Label distribution in this split:")
        print(self.metadata[self.target_labels].value_counts())
    ...
    def __getitem__(self, idx):
        ...
        #print(f"[DEBUG] Dataset root_dir = {self.root_dir}")
        #print(f"[DEBUG] Trying to load image from: {img_path}")
        channel_width = self.metadata.loc[idx, "channel_width"]
        image = skimage.io.imread(img_path)
        image = self._fold_channels(image, channel_width)
        #print("[DEBUG] Running __getitem__ from", __file__)

        if self.is_train or self.include_labels:
            label_str = self.metadata.loc[idx, self.target_labels]

            if label_str not in self.train_classes_dict:
                print(f"[WARNING] Unknown label: {label_str}")
                label = -1  # or you can `return None` or `continue` depending on your setup
            else:
                label = self.train_classes_dict[label_str]
        ...
        #         label = torch.tensor(label)
        #         #label = self.train_classes_dict[label]
        #         # label = self.train_classes_dict.get(label, -1)
        #         # label = torch.tensor(label)
        #         # if label not in self.train_classes_dict:
        #         #     print(f"[WARNING] Unknown label: {label}")

        #         #print(f"[DEBUG] idx={idx}, label={label}")
        #         #print("DEBUG: include_labels =", self.include_labels)
        #         # print(f"[DEBUG] Getting label at idx={idx}")
        #         # print(f"[DEBUG] include_labels={self.include_labels}, is_train={self.is_train}")
        #         # print(f"[DEBUG] self.metadata columns: {self.metadata.columns}")
        #         # print(f"[DEBUG] self.target_labels: {self.target_labels}")
        #         # print(f"[DEBUG] label raw: {self.metadata.loc[idx, self.target_labels]}")

        else:
            label = None
            #print(f"[DEBUG] idx={idx}, label={label}")
            print("DEBUG: label was set to None")
        ...
        if self.chunk == "morphem70k" or self.chunk not in [
            ...
            #if self.is_train:
            if self.is_train or self.include_labels: #changed 4/6
