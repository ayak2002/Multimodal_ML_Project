...
from diverse_channel_vit.models.fan_transformer_block import FANBlock
...
class PatchEmbedPerChannel(nn.Module):
    ...
    def __init__(
        ...
        fan_enabled: bool = False,  # New parameter to enable FAN-inspired channel attention
    ):
        super().__init__()
        self.cfg = config
        ...
        if self.use_channelvit_channels:
            # Original channel attention mechanism (kept for backward compatibility but commented out)
            ...
            # FAN-inspired enhanced channel attention
            self.use_fan_attention = fan_enabled  # Default to False for backward compatibility
            self.fan_channel_attention = EnhancedChannelAttention(
                embed_dim=embed_dim,
                num_heads=4,
                dropout=0.0,
                reduction_ratio=4
            ...
            # Flag to track diversity loss
            self.channel_diversity_loss = 0.0
            
            # Set this flag to ensure we only print the channel attention type once
            self._printed_channel_attention_type = False
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.use_channelvit_channels:
            ...
            # Only print once during initialization to avoid spam
            if not hasattr(self, '_printed_channel_attention_type'):
                if self.use_fan_attention:
                    print("[PatchEmbed] Using FAN-inspired channel attention for robust channel reweighting")
                else:
                    print("[PatchEmbed] Using standard channel reweighting")
                self._printed_channel_attention_type = True
            ...
            if self.use_fan_attention:
                scores, diversity_loss = self.fan_channel_attention(channel_embed_perm)
                self.channel_diversity_loss = diversity_loss
            else:
                # Fall back to simple attention if FAN is not enabled
                # Use a simple attention mechanism since the original is commented out
                scores = torch.ones(x.shape[0], Cin, device=x.device)  # Equal weighting as fallback
            ...
            if not self.training:
                ...
                out_dir = "/projectnb/cs598/projects/Modalities_Robustness/adaptive_channel_strategy_aya/attention_logs"
        ...
        return x, Cin, ortho_proxy_loss + (self.channel_diversity_loss if hasattr(self, 'channel_diversity_loss') and self.use_fan_attention else 0)
...
class ChannelVisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.patch_embed = PatchEmbedPerChannel(
            ...
            fan_enabled=kwargs.get('fan_enabled', False),  # Get fan_enabled from kwargs
        )
        ...
        if self.cfg.block_type == "block_v2":
            ...
            # Use FANBlock if fan_enabled is True, otherwise use standard Block
            if kwargs.get('fan_enabled', False):
                BlockClass = FANBlock
                print("Using FAN-inspired channel attention in transformer blocks")
            else:
                BlockClass = Block
        else:
        ...
        self.blocks = nn.ModuleList(
            [
                BlockClass(
                    ...
                    **(kwargs if BlockClass == FANBlock else {k: v for k, v in kwargs.items() if k != 'fan_enabled'})
                )
    ...
    def prepare_tokens(self, x, chunk: str, training_chunks_str, new_channel_init, extra_tokens):
        ...
        if self.cfg.dropout_tokens_hcs == "random" and self.training:  ## x: [B CinHW Cout]
            cinHW = x.shape[1]
            HW = cinHW // nc
            cinHW_new = random.randint(1, cinHW)
    ...
    def forward(
        ...
        # Handle dictionary inputs from adaptive datasets
        if isinstance(x, dict):
            if 'image' in x:
                img = x['image']
                B, _, w, h = img.shape
                img, nc, ortho_proxy_loss = self.patch_embed(
                    img, chunk_name, training_chunks, new_channel_init, extra_tokens
                )
                nc = img.shape[1] // ((w // self.patch_size) * (h // self.patch_size))
                
                # Track diversity loss from FAN blocks
                fan_diversity_loss = 0.0
                
                for blk in self.blocks:
                    if isinstance(blk, BlockV2):
                        img, counter = blk(img, pruning_method=self.cfg.dropout_tokens_hcs, nc=nc)
                    else:
                        img = blk(img)
                        # Collect diversity loss if using FANBlock
                        if hasattr(blk, 'fan_enabled') and blk.fan_enabled and hasattr(blk, 'channel_diversity_loss'):
                            fan_diversity_loss += blk.channel_diversity_loss
                
                img = self.norm(img)
                # Add FAN diversity loss to ortho_proxy_loss if present
                if fan_diversity_loss > 0:
                    ortho_proxy_loss = ortho_proxy_loss + fan_diversity_loss
                return img[:, 0].clone(), ortho_proxy_loss
            else:
                # Handle case where 'image' key is not present
                raise ValueError("Dictionary input does not contain 'image' key")
        else:
            # Original behavior for tensor inputs
            B, _, w, h = x.shape
            x, nc, ortho_proxy_loss = self.patch_embed(
...
def channelvit_distill(config, patch_size=14, in_chans=0, mapper=None, **kwargs):
    # Remove fan_enabled from kwargs if present
    if 'fan_enabled' in kwargs:
        kwargs.pop('fan_enabled')
...
class DiChaViTAdaptive(nn.Module):
    def __init__(self, config: Model, **kwargs):
        ...
        # Extract fan_enabled from config
        fan_enabled = getattr(config, 'fan_enabled', False)
        
        # Print a clear message about FAN status
        if fan_enabled:
            print("\n" + "="*80)
            print("INITIALIZING DiChaViT WITH FAN-INSPIRED CHANNEL ATTENTION ENABLED")
            print("This will enhance cross-channel information sharing and feature diversity")
            print("="*80 + "\n")
