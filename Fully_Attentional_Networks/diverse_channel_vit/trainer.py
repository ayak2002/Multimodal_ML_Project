...
from channel_adaptive_models.MorphEm.morphem.benchmark import run_benchmark
...
class Trainer:
    ...
    def eval_subset_channels(self):
        ...
        #index 4/6
            ...
            chunk_channels = None
            for chunk_dict in self.cfg.data_chunk["chunks"]:
                if chunk_name in chunk_dict:
                    chunk_channels = chunk_dict[chunk_name]
                    break

            if chunk_channels is None:
                raise ValueError(f"Chunk name '{chunk_name}' not found in data_chunk config.")
    ...
    def eval_morphem70k(
        ...
        for chunk_name in eval_chunks:
            ...
            for bid, batch in tqdm(enumerate(eval_loader), total=total_iterations):
                ...
                # Check if x is a dictionary (from the adaptive dataset) or a tensor
                if isinstance(x, dict):
                    # For adaptive dataset, use the image key
                    if 'image' in x:
                        img = x['image']
                        self.model.current_selected_channels = (
                            channel_combinations if channel_combinations is not None
                            else list(range(img.shape[1]))  # default to all channels
                        )
                        
                        if channel_combinations is not None:
                            img = img[:, channel_combinations, :, :].clone()
                            x['image'] = img
                    else:
                        # If no image key, use default channels
                        self.model.current_selected_channels = (
                            channel_combinations if channel_combinations is not None
                            else list(range(12))  # default to all 12 channels for MorphEm
                        )
                else:
