...
subset_indices_within_allen = [1,2]  # for example
allen_mapper = [0,1,2]  # current Allen
selected_channels = [allen_mapper.index(i) for i in subset_indices_within_allen]  # → 

if len(selected_channels) != len(subset_indices_within_allen):
    raise ValueError(f"Some indices in {subset_indices_within_allen} are not in allen_mapper {allen_mapper}")
...
cfg.eval.dest_dir = f"/projectnb/cs598/projects/Modalities_Robustness/adaptive_channel_strategy/results/subset_eval_allen_{'_'.join(map(str, selected_channels))}"
...
trainer.model.feature_extractor.current_chunk_name = "Allen"
trainer.model.feature_extractor.current_epoch = 60

#trainer.model.feature_extractor.mapper["CP"] = subset_indices_within_cp
trainer.model.feature_extractor.patch_embed.mapper["Allen"] = subset_indices_within_allen
# === Run evaluation ===
trainer.eval_morphem70k(epoch=60, new_channel_init="",  eval_chunks=["Allen"])
