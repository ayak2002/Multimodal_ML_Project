...
def create_umap(dataset, features_path, df_path, dest_dir, label_list):
    ...
    print(f"Metadata path: {df_path}")

    features = np.load(features_path)
    print(f"Features loaded with shape: {features.shape}")
    ...
    required_columns = ['train_test_split', 'Label']
    ...
    print(f"Task columns found: {task_columns}")
...
def evaluate(features_path, df_path, leave_out, leaveout_label, model_choice, use_gpu: bool, knn_metric: str):
    ...
    if leave_out != None:
        ...
        print(f"Leaving out task: {leave_out} at index {leaveout_ind}")
    ...
    # Debug the task columns
    for task in tasks:
        print(f"Task {task}: {sum(df[task])} samples")
    
    all_test_indices = [np.where(df[task])[0] for task in tasks]
    print(f"Number of test sets: {len(all_test_indices)}")
    ...
    for i, test_indices in enumerate(all_test_indices):
        print(f"Task {i+1}: Number of test indices: {len(test_indices)}")
        if len(test_indices) > 0:
            print(f"Task {i+1}: Min index: {min(test_indices)}, Max index: {max(test_indices)}")
        else:
            print(f"Task {i+1}: Test indices is empty!")
    ...
    for i, test_indices in enumerate(all_test_indices):
        try:
            ...
            print(f"Task {i+1}: Successfully extracted features with shape {test_X.shape}")
        except IndexError as e:
            print(f"Task {i+1}: IndexError: {e}")
            print(f"Task {i+1}: Features shape: {features.shape}, Max index in test_indices: {max(test_indices) if len(test_indices) > 0 else 'N/A'}")
            # Create empty array with same shape as features except first dimension
            empty_shape = list(features.shape)
            empty_shape[0] = 0
            test_Xs.append(np.zeros(empty_shape, dtype=features.dtype))
            print(f"Task {i+1}: Created empty array with shape {empty_shape}")
