...
from morphem_fixed.benchmark import run_benchmark
...
class Trainer:
    ...
    #     @return:
    #     """
    #     kwargs["return_shared_specific"] = True
    #     if self.forward_mode == "need_chunk_name":
    #         shared_output, specific_output, ortho_proxy_loss = self.model(
    ...
    #             #return_shared_specific=True,  #avantika added
    #             **kwargs,
    #         )
    #     else:
    #         #output = self.model(x)
    #         shared_output, specific_output, ortho_proxy_loss = self.model(x, **kwargs) #avantika added
            
    #     return shared_output, specific_output, ortho_proxy_loss #output
    # commented this whole thing for now and replacing it wit this:
    def _forward_model(
        self,
        x,
        chunk_name: str | None,
        ...
        **kwargs,
        ...
        return_shared_specific = kwargs.pop("return_shared_specific", False)
        ...
        if self.forward_mode == "need_chunk_name":
            output = self.model(
                ...
                init_first_layer=init_first_layer,
                new_channel_init=new_channel_init,
                return_shared_specific=return_shared_specific,
                **kwargs,
            )
        elif self.forward_mode == "need_training_chunks":
            output = self.model(
            ...
            output = self.model(x, return_shared_specific=return_shared_specific, **kwargs)

        # === Return format based on flag ===
        if return_shared_specific:
            if isinstance(output, tuple) and len(output) == 3:
                return output  # (shared, specific, loss)
            else:
                # If model incorrectly returns 1 tensor, make shared = specific
                return output, output, None
        else:
            return output, None, None
        # if return_shared_specific:
        #     return output  # already (shared, specific, loss)
        # else:
        #     return output, None, None  # pad to keep return format consistent
    ...
    def eval_morphem70k(
        ...
        def log_res(eval_cfg, knn_metric, dataset_name=None):
            call_umap = eval_cfg["umap"] and (epoch == 0 or epoch == self.cfg.train.num_epochs)
            if eval_chunks is None or len(eval_chunks) == 3:
                dataset = self.cfg.dataset.name if dataset_name is None else dataset_name
            else:
                dataset = eval_chunks[0] if dataset_name is None else dataset_name  ## TODO: for now only test on 1 transfered chunk or all chunks
            
            print(f"Dataset: {dataset}")
            # Use the feature path for the current dataset being evaluated
            feature_dir = os.path.join(eval_cfg["feature_dir"])
            feature_path = os.path.join(feature_dir, eval_cfg["feature_file"])
            print(f"features_path: {feature_path}")
            
            # Get the metadata path for the current dataset
            metadata_path = os.path.join(eval_cfg["root_dir"], dataset, "enriched_meta.csv")
            print(f"df_path: {metadata_path}")
            ...
            if knn_metric in ["l2", "cosine"]:
                try:
                    # Use the specific feature path and metadata path for the current dataset
                    full_res = run_benchmark(
                        eval_cfg["root_dir"],
                        eval_cfg["dest_dir"],
                        eval_cfg["feature_dir"],  # Pass the BASE feature directory
                        eval_cfg["feature_file"], # Pass the feature filename
                        eval_cfg["classifier"],
                        call_umap,
                        eval_cfg["use_gpu"],
                        knn_metric,
                        dataset,  # Pass the current dataset name
                    )
                    ## log results
                    ...
                    print(f"Error evaluating {dataset} with {knn_metric}: {str(e)}")
                    metrics_logger = {}
            else:
                knn_metric = "l2"
                try:
        ...
        for chunk_name in eval_chunks:
            ...
            shared_outputs = [] #added avantika
            specific_outputs = [] #added avantika
            ...
            for bid, batch in tqdm(enumerate(eval_loader), total=total_iterations):
                ...
                #     new_channel_init=new_channel_init,
                #     # idx=bid,
                #     return_shared_specific=True, #avantika adding temporarily
                # )
                outputs = self._forward_model(
                    ...
                    new_channel_init=new_channel_init,
                    # idx=bid,
                    return_shared_specific=True, #avantika adding temporarily commented out first block for vis
                )
                if isinstance(outputs, tuple) and len(outputs) == 3:
                    #shared_output, specific_output, _ = outputs
                    print(type(outputs), len(outputs))
                    print(type(outputs[0]), len(outputs[0]))
                    #(shared_output, specific_output), _ = outputs
                    (shared_output, specific_output), loss, disentanglement_losses = outputs


                else:
                    raise ValueError("Expected 3 outputs (shared, specific, loss), got something else")
                #     shared_output = outputs
                #     specific_output = outputs
                # if isinstance(shared_output, tuple):
                #     shared_output = shared_output[0]
                # if isinstance(specific_output, tuple):
                #     specific_output = specific_output[0]
                #shared_output = shared_output[:, 0]  # ← Get CLS token
                #specific_output = specific_output[:, 0]  # ← Get CLS token
                shared_output = shared_output.float()
                specific_output = specific_output.float()
                # fused_output = 0.5 * (shared_output + specific_output)
                ...
                #fused_output = 0.5 * (shared_output + specific_output)
                # feat_outputs.append(fused_output)

            torch.cuda.empty_cache()
            gc.collect()
            #feat_outputs = torch.cat(feat_outputs, dim=0).cpu().numpy()     #avantika
                
            shared_features = torch.cat(shared_outputs, dim=0).cpu().numpy() #avantika
            specific_features = torch.cat(specific_outputs, dim=0).cpu().numpy() #avantika
            ...
            out_path_shared = os_join(folder_path, f"shared_{eval_cfg.feature_file}") #avantika
            out_path_specific = os_join(folder_path, f"specific_{eval_cfg.feature_file}") #avantika
            out_path_list.append(out_path)
            
            # Ensure the directory exists before saving features
            os.makedirs(folder_path, exist_ok=True)
            
            #utils.write_numpy(feat_outputs, out_path)
            utils.write_numpy(shared_features, out_path_shared)
            utils.write_numpy(specific_features, out_path_specific)
        ...
        # Initialize final_scores to store all evaluation results
        final_scores = {}
        
        # Process each dataset separately to ensure correct feature-metadata matching
        for chunk_name in eval_chunks:
            print(f"\nEvaluating {chunk_name} dataset...")
            for classifier in eval_cfg.classifiers:
                eval_cfg.classifier = classifier
                
                if classifier == "knn":
                    for knn_metric in eval_cfg.knn_metrics:
                        print(f"Running {classifier} with {knn_metric} metric on {chunk_name}")
                        try:
                            if "cosine" in knn_metric:
                                metrics = log_res(eval_cfg=eval_cfg, knn_metric=knn_metric, dataset_name=chunk_name)
                                cosine_metrics = metrics
                                # Update final_scores with the results
                                if metrics:
                                    final_scores.update(metrics)
                            else:
                                metrics = log_res(eval_cfg=eval_cfg, knn_metric=knn_metric, dataset_name=chunk_name)
                                # Update final_scores with the results
                                if metrics:
                            ...
                            print(f"Error evaluating {chunk_name} with {knn_metric}: {str(e)}")
                else:
                    try:
                        metrics = log_res(eval_cfg=eval_cfg, knn_metric=None, dataset_name=chunk_name)
                        # Update final_scores with the results
                        if metrics:
                            final_scores.update(metrics)
                    except Exception as e:
                        print(f"Error evaluating {chunk_name} with {classifier}: {str(e)}")
