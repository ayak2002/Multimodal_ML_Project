...
class PPTAttention(Attention):
    ...
    def score_assignment_step(attn, v):
        ...
        return None #significance_score
    ...
    # confirm cls tokens are reserved
        ...
        # cls_score = constant * torch.ones(scores.shape[0], 1).to(scores.device)
        # scores = torch.cat([cls_score, scores], dim=-1)  # [B, N] avantika commented and added the above for eval 
...
class BlockV2(nn.Module):
    ...
    def forward(self, x, return_attention=False, pruning_method=None, nc: int = 0):
        print(">> BlockV2.forward(): pruning_method =", pruning_method)
        #y, scores = self.attn(self.norm1(x))
        y, scores, attn_map = self.attn(self.norm1(x))
        self.attn.attn_map = attn_map  # Save it for later visualization
        ...
        if pruning_method is not None:
            ...
            elif pruning_method == "none" or pruning_method is None:
                # no pruning, just pass avantika
                pass
