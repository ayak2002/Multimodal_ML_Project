        ...
        **kwargs,
        ...
        if self.training and self.enable_sample:
            ...
            if self.cfg.hcs_sampling == "none" or self.cfg.hcs_sampling is None:
                ...
                # Check if we're using HCS sampling with fission module
                if not self.use_channelvit_channels and self.cfg.hcs_sampling:
                    pass
                else:
                    assert (
            ...
            for c in self.mapper[chunk_name]:
                if c not in training_channels:
                    if new_channel_init in [avg2, avg2_not_seen]:
                        ...
                        xc_test = F.normalize(xc_test, p=2, dim=-1)
                        bank = F.normalize(bank, p=2, dim=-1)
                        corr = torch.einsum("b d, c h ->b c", xc_test, bank)
        ...
        if self.cfg.proxy_loss_lambda > 0 and self.use_channelvit_channels:
...
class ChannelVisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.mapper = mapper  # Store mapper as an attribute
        
        # Determine if we should use channel embeddings
        # If fission module is enabled, we don't need channel embeddings
        use_fission_module = getattr(config, 'use_fission_module', False)
        if use_fission_module:
            use_channelvit_channels = False
        
        # Debug prints for fission/fusion configuration
        print("\n==== DiChaViT Configuration Debug ====")
        print(f"use_fission_module: {use_fission_module}")
        print(f"use_parallel_paths: {getattr(config, 'use_parallel_paths', False)}")
        print(f"use_fusion_module: {getattr(config, 'use_fusion_module', False)}")
        print(f"feature_separation_lambda: {getattr(config, 'feature_separation_lambda', 0.0)}")
        print(f"adversarial_lambda: {getattr(config, 'adversarial_lambda', 0.0)}")
        print(f"fission_hidden_dim: {getattr(config, 'fission_hidden_dim', None)}")
        print(f"fission_shared_dim: {getattr(config, 'fission_shared_dim', None)}")
        print(f"fission_specific_dim: {getattr(config, 'fission_specific_dim', None)}")
        print("========================================\n")
        ...
        # Initialize fission module if enabled
        self.use_fission_module = getattr(config, 'use_fission_module', False)
        self.use_parallel_paths = getattr(config, 'use_parallel_paths', False) and self.use_fission_module
        self.share_transformer_weights = getattr(config, 'share_transformer_weights', True)
        self.use_fusion_module = getattr(config, 'use_fusion_module', False) and self.use_parallel_paths
        
        # Initialize loss weights
        self.feature_separation_lambda = getattr(config, 'feature_separation_lambda', 0.0)
        self.adversarial_lambda = getattr(config, 'adversarial_lambda', 0.0)
        self.adversarial_scaling_factor = getattr(config, 'adversarial_scaling_factor', 100.0)
        
        if self.use_fission_module:
            hidden_dim = config.fission_hidden_dim or embed_dim
            # Ensure shared and specific dimensions match embed_dim for compatibility with transformer blocks
            shared_dim = config.fission_shared_dim or embed_dim
            specific_dim = config.fission_specific_dim or embed_dim
            self.fission_module = FissionModule(embed_dim, hidden_dim, shared_dim, specific_dim)
            
            # Create fusion module if enabled
            if self.use_fusion_module:
                fusion_hidden_dim = config.fusion_hidden_dim or (shared_dim + specific_dim)
                fusion_out_dim = config.fusion_out_dim or embed_dim
                self.fusion_module = FusionModule(shared_dim, specific_dim, fusion_hidden_dim, fusion_out_dim)
            
            # Create channel classifier for adversarial training if enabled
            if self.adversarial_lambda > 0:
                self.channel_classifier = ChannelClassifier(shared_dim, in_chans, alpha=self.adversarial_lambda)
            else:
                self.channel_classifier = None
            
            # Create separate transformer blocks for specific features if not sharing weights
            if self.use_parallel_paths and not self.share_transformer_weights:
                self.specific_blocks = nn.ModuleList(
                    [
                        BlockClass(
                            dim=embed_dim,
                            num_heads=num_heads,
    ...
    def interpolate_pos_encoding(self, x, w, h, nc):
        ...
        # Calculate w0 and h0 early so they're available for all code paths
        ...
        # For fission module case, we need a completely different approach
        if hasattr(self, 'use_fission_module') and self.use_fission_module:
            # When fission is enabled, we need to match exactly the number of tokens in x
            class_pos_embed = self.pos_embed[:, :num_extra_tokens]
            
            # Create a new positional embedding that matches the exact size of x
            if npatch > N:
                # We need to expand the positional embeddings
                ...
                # If still not matching, interpolate again to match exactly
                if patch_pos_embed.shape[1] != npatch:
                    patch_pos_embed = nn.functional.interpolate(
                        patch_pos_embed.unsqueeze(0),
                        size=(npatch, dim),
                        mode="bilinear",
                        align_corners=False
                    ).squeeze(0)
        ...
        # Original implementation for non-fission case
        if npatch == N and w == h:
        ...
        # w0 and h0 are already calculated above
        patch_pos_embed = nn.functional.interpolate(
            patch_pos_embed.reshape(1, int(math.sqrt(N)), int(math.sqrt(N)), dim).permute(0, 3, 1, 2),
            scale_factor=(w0 / math.sqrt(N), h0 / math.sqrt(N)),
            mode="bicubic",
        ...
        # Create a positional embedding that exactly matches the number of patches
        if nc == 1 and npatch != patch_pos_embed.shape[1]:
            # Direct approach: create a positional embedding that exactly matches npatch
            patch_pos_embed = nn.functional.interpolate(
                patch_pos_embed.unsqueeze(0),
                size=(npatch, dim),
                mode="bilinear",
            ...
            # Original approach: duplicate for each channel
            patch_pos_embed = patch_pos_embed.expand(1, nc, -1, dim).reshape(1, -1, dim)
            
            # If still not matching, resize to match exactly
            if patch_pos_embed.shape[1] != npatch:
                patch_pos_embed = nn.functional.interpolate(
                    patch_pos_embed.unsqueeze(0),
                    size=(npatch, dim),
    ...
    def prepare_tokens(self, x, chunk: str, training_chunks_str, new_channel_init, extra_tokens):
        B, _, w, h = x.shape  # Original input shape
        ...
        # Ensure ortho_proxy_loss is a tensor, not a float
        if isinstance(ortho_proxy_loss, float):
            ortho_proxy_loss = torch.tensor(ortho_proxy_loss, device=x.device)
    ...
    def forward(
        ...
        extra_tokens={},
        return_shared_specific: bool = False,  # NEW FLAG avantika added
    ):
        # Get embeddings from patch embedding layer
        B, C, h, w = x.shape  # Original input shape
        x, ortho_proxy_loss = self.prepare_tokens(
            x, chunk_name, training_chunks, new_channel_init, extra_tokens
        )
        
        # After prepare_tokens, x has shape [B, tokens, features]
        # For BlockV2, we need to know the number of channels (nc)
        nc = self.in_chans if self.patch_embed.use_channelvit_channels else 1

        # Apply fission module if enabled
        if self.use_fission_module and self.fission_module is not None:
            # Only print from the main GPU in DataParallel
            # Use a static counter to track batches and only print from device 0
            if not hasattr(DiChaViT, '_batch_counter'):
                DiChaViT._batch_counter = 0
                DiChaViT._print_device = True
            
            # Only the first GPU in the batch should print
            is_print_device = DiChaViT._print_device
            if is_print_device:
                DiChaViT._print_device = False
                print(f"Forward pass - training mode: {self.training}")
            
            shared_features, specific_features = self.fission_module(x)
            
            # Process features through parallel paths if enabled
            if self.use_parallel_paths:
                # Process shared features through main transformer blocks
                shared_x = shared_features
                for blk in self.blocks:
                    if isinstance(blk, BlockV2):
                        #shared_x, counter = blk(shared_x, pruning_method=self.cfg.dropout_tokens_hcs, nc=nc) avantika commented
                        if self.cfg.dropout_tokens_hcs != "none":
                            shared_x, counter = blk(shared_x, pruning_method=self.cfg.dropout_tokens_hcs, nc=nc)
                        else:
                            shared_x = blk(shared_x, pruning_method="none", nc=nc)

                    else:
                        shared_x = blk(shared_x)
                
                # Process specific features through transformer blocks (shared or separate)
                specific_x = specific_features
                if self.share_transformer_weights:
                    # Use the same blocks for specific features
                    for blk in self.blocks:
                        if isinstance(blk, BlockV2):
                            #specific_x, counter = blk(specific_x, pruning_method=self.cfg.dropout_tokens_hcs, nc=nc)
                            if self.cfg.dropout_tokens_hcs != "none":
                                specific_x, counter = blk(specific_x, pruning_method=self.cfg.dropout_tokens_hcs, nc=nc)
                            else:
                                specific_x = blk(specific_x, pruning_method="none", nc=nc)
                        else:
                            specific_x = blk(specific_x)
                else:
                    # Use separate blocks for specific features
                    for blk in self.specific_blocks:
                        if isinstance(blk, BlockV2):
                            #specific_x, counter = blk(specific_x, pruning_method=self.cfg.dropout_tokens_hcs, nc=nc)
                            if self.cfg.dropout_tokens_hcs != "none":
                                specific_x, counter = blk(specific_x, pruning_method=self.cfg.dropout_tokens_hcs, nc=nc)
                ...
                # Apply normalization to both feature paths
                print("=== After transformer blocks ===")
                print("Mean of shared_x:", shared_x.mean().item())
                print("Mean of specific_x:", specific_x.mean().item())
                print("Mean absolute difference:", (shared_x - specific_x).abs().mean().item())
                print("================================")

                shared_x = self.norm(shared_x)
                specific_x = self.norm(specific_x)
                
                # Compute additional losses for feature disentanglement
                additional_losses = 0
                disentanglement_losses = {}
                
                # Feature separation loss
                if self.feature_separation_lambda > 0 and self.training:
                    if is_print_device:
                        print(f"Calculating feature separation loss (lambda={self.feature_separation_lambda})")
                    
                    sep_loss = feature_separation_loss(shared_x, specific_x)
                    additional_losses += self.feature_separation_lambda * sep_loss
                    disentanglement_losses['feature_separation_loss'] = sep_loss.item()
                    
                    # Print loss only from the main GPU
                    if is_print_device:
                        print(f"Feature separation loss: {sep_loss.item():.4f}")
                else:
                    # Add zero loss to ensure it appears in wandb
                    disentanglement_losses['feature_separation_loss'] = 0.0
                
                # Adversarial channel loss
                if self.adversarial_lambda > 0 and self.channel_classifier is not None and self.training:
                    # Extract actual channel indices from the input data
                    batch_size = shared_x.shape[0]
                    
                    # For adversarial training, we need to identify which channel each sample belongs to
                    # Since we're using the CLS token for classification, we can use it for the adversarial loss too
                    if self.mapper and chunk_name in self.mapper:
                        # Get all channels for this chunk
                        chunk_channels = self.mapper[chunk_name]
                        num_channels = len(chunk_channels)
                        
                        # Create channel indices that cycle through all available channels
                        channel_indices = torch.tensor(
                            [chunk_channels[i % num_channels] for i in range(batch_size)],
                            dtype=torch.long, 
                            device=shared_x.device
                        )
                    else:
                        # Fallback to zeros if we can't determine the channels
                        channel_indices = torch.zeros(batch_size, dtype=torch.long, device=shared_x.device)
                    
                    # Compute adversarial loss
                    adv_loss = adversarial_channel_loss(shared_x, channel_indices, self.channel_classifier, scaling_factor=self.adversarial_scaling_factor)
                    
                    # With gradient reversal, we just add the loss directly
                    # The gradients will be automatically reversed during backpropagation
                    additional_losses += self.adversarial_lambda * adv_loss
                    disentanglement_losses['adversarial_channel_loss'] = adv_loss.item()
                    
                    # Print loss only once per batch
                    if is_print_device:
                        print(f"Adversarial channel loss: {adv_loss.item():.4f}")
                else:
                    # Add zero losses to ensure they appear in wandb
                    disentanglement_losses['adversarial_channel_loss'] = 0.0
                
                # Proxy loss using specific features
                if self.patch_embed.cfg.proxy_loss_lambda > 0 and self.training:
                    # Get the CLS token from specific features
                    specific_cls = specific_x[:, 0]  # Shape: [batch_size, specific_dim]
                    
                    # Create channel ground truth
                    if self.mapper and chunk_name in self.mapper:
                        chunk_channels = self.mapper[chunk_name]
                        num_channels = len(chunk_channels)
                        
                        # Get the total number of channels from the channel_emb_proxies
                        total_channels = self.patch_embed.channel_emb_proxies.shape[0]
                        
                        # Create one-hot ground truth for channels
                        channel_gt = torch.zeros(batch_size, total_channels, device=specific_x.device)
                        for i in range(batch_size):
                            channel_idx = chunk_channels[i % num_channels]
                            channel_gt[i, channel_idx] = 1.0
                        
                        # Get channel proxies
                        channel_emb_proxies = self.patch_embed.channel_emb_proxies
                        ...
                        proxy_loss_val = proxy_loss(
                            ...
                            specific_cls, 
                            channel_gt, 
                            scale=self.patch_embed.channel_scale
                        )
                        
                        # Add to total loss
                        additional_losses += self.patch_embed.cfg.proxy_loss_lambda * proxy_loss_val
                        disentanglement_losses['proxy_loss'] = proxy_loss_val.item()
                        
                        # Print loss only once per batch
                        if is_print_device:
                            print(f"Proxy loss: {proxy_loss_val.item():.4f}")
                else:
                    # Add zero loss to ensure it appears in wandb
                    disentanglement_losses['proxy_loss'] = 0.0
                
                # Add additional losses to the ortho_proxy_loss
                ortho_proxy_loss += additional_losses
                
                # Reset print flags at the end of each batch
                if self.training:
                    # Increment batch counter and reset print device flag for the next batch
                    if hasattr(DiChaViT, '_batch_counter'):
                        DiChaViT._batch_counter += 1
                        # Reset print device flag every 20 batches
                        if DiChaViT._batch_counter % 20 == 0:
                            DiChaViT._print_device = True
                if return_shared_specific:
                #return both shared and specific paths before fusion avantika added for visulaization
                    #return shared_x[:, 0].clone(), specific_x[:, 0].clone(), ortho_proxy_loss
                    return shared_x[:, 0].clone(), specific_x[:, 0].clone(), {} #avantika doing this for vis
                
                # Use fusion module to combine shared and specific features
                if self.use_fusion_module and self.fusion_module is not None:
                    x = self.fusion_module(shared_x, specific_x)
                else:
                    x = shared_x
                ...
                # If parallel paths are disabled, just use the shared features
                x = shared_features
                for blk in self.blocks:
                    x = blk(x, nc=nc)
                x = self.norm(x)
                ...
                # Return empty disentanglement losses dictionary
                disentanglement_losses = {}
                return x, ortho_proxy_loss
        else:
            # Standard processing without fission
            for blk in self.blocks:
                x = blk(x, nc=nc)
            x = self.norm(x)
...
class DiChaViT(nn.Module):
    ...
    def forward(
        ...
        return_shared_specific: bool = False,
        **kwargs,
    ) -> torch.Tensor:
        # init_first_layer: not used
        #output = self.feature_extractor(x, chunk_name, training_chunks, new_channel_init)       avantika
        output = self.feature_extractor(
            x, chunk_name, training_chunks, new_channel_init, return_shared_specific=return_shared_specific,
        )
        
        
        # Handle different return formats from feature_extractor
        if isinstance(output, tuple):
            if len(output) == 3:  # During training: (features, ortho_proxy_loss, disentanglement_losses)
                x, ortho_proxy_loss, disentanglement_losses = output
                
                # Ensure all values in disentanglement_losses are tensors
                for k, v in disentanglement_losses.items():
                    if isinstance(v, (int, float)):
                        disentanglement_losses[k] = torch.tensor(v, device=x.device)
                
                x = self.classifer_head(x)
                return x, ortho_proxy_loss
            else:  # During evaluation with older format: (features, ortho_proxy_loss)
                x, ortho_proxy_loss = output
                x = self.classifer_head(x)
                return x
        else:  # During evaluation with new format: just features
            # Return the raw features without applying classifier head
            return output
...
class FissionModule(nn.Module):
    """
    Splits input features into shared and channel-specific components via two MLPs.
    Configurable via num_hidden and output dimension.
    """
    def __init__(self, feature_dim, hidden_dim=None, shared_dim=None, specific_dim=None):
        super().__init__()
        hidden_dim = hidden_dim or feature_dim
        shared_dim = shared_dim or feature_dim // 2
        specific_dim = specific_dim or feature_dim // 2
    ...
    def forward(self, x):
        ...
        specific = self.specific_mlp(x)
        print("Mean of shared features:", shared.mean().item())
        print("Mean of specific features:", specific.mean().item())
        print("Difference:", (shared - specific).abs().mean().item())
        return shared, specific


class FusionModule(nn.Module):
    """
    Combines shared and channel-specific features.
    Uses simple averaging of shared and specific features.
    """
    def __init__(self, shared_dim, specific_dim, fusion_hidden_dim=None, fusion_out_dim=None):
        super().__init__()
        # Store dimensions for potential future use
        self.shared_dim = shared_dim
        self.specific_dim = specific_dim
        
        # If dimensions don't match, we'll need projection layers
        self.need_projection = shared_dim != specific_dim
        if self.need_projection:
            # Project to the larger dimension
            self.target_dim = max(shared_dim, specific_dim)
            if shared_dim < specific_dim:
                self.shared_proj = nn.Linear(shared_dim, specific_dim)
            else:
                self.specific_proj = nn.Linear(specific_dim, shared_dim)

    def forward(self, shared, specific):
        # shared: (batch, tokens, shared_dim)
        # specific: (batch, tokens, specific_dim)
        
        # If dimensions don't match, project to the same dimension
        if self.need_projection:
            if self.shared_dim < self.specific_dim:
                shared = self.shared_proj(shared)
            else:
                specific = self.specific_proj(specific)
        
        # Simple averaging of features
        fused = (shared + specific) / 2.0
