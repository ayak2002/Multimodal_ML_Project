...
from diverse_channel_vit.datasets.cifar import CifarRandomInstance
from diverse_channel_vit.datasets.tps_transform import TPSTransform, dummy_transform
...
def get_mean_std_dataset(dataset):
    ...
    mean_hpa, std_hpa = (
        [0.08290475, 0.04112732, 0.064044476, 0.08445487],
        [0.08106554, 0.052796874, 0.0885671, 0.0815554],
    )
    mean_cp, std_cp = (
        [0.09957533, 0.19229738, 0.16250879, 0.18240248, 0.14978176],
...
def get_data_transform(dataset: str, img_size: int, tps_prob: float, ssl_flag: bool):
    ...
    if ssl_flag:
        if dataset in ["Allen", "CP", "HPA"]:
            ...
            for data in ["CP", "Allen", "HPA"]:
                transform_train[data] = ContrastiveLearningViewGenerator(
                    transform_train[data], n_views=2
                )
...
def get_samplers(dataset: str, img_size: int, chunk_name: str, train_split: bool) -> Tuple:
    ...
    if dataset in ["cifar10", "cifar100"]:
        ...
        data_set = torch_dataset(
            root="./data", train=train_split, download=True, transform=transform_train
        )
    ...
    if dataset in ["cifar10", "cifar100"]:
        ...
        if dataset == "Allen":
            if "morphem70k_v2" in file_name:
                train_classes = ["M0", "M1M2", "M3", "M4M5", "M6M7_complete", "M6M7_single"]
            else:
...
def make_random_instance_train_loader(
    ...
    if dataset in ["cifar10", "cifar100"]:
        return make_cifar_random_instance_train_loader(
            dataset, img_size, batch_size, seed, num_workers
        )
