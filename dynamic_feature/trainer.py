...
class Trainer:
    ...
    def eval_morphem70k(
        ...
        def log_res(eval_cfg, knn_metric):
            ...
            if knn_metric in ["l2", "cosine"]:
                ...
                dest_dir=eval_cfg["dest_dir"],
                feature_dir=eval_cfg["feature_dir"],
                feature_file=eval_cfg["feature_file"],
                classifier=eval_cfg["classifier"],
                call_umap=call_umap,
    ...
    @torch.inference_mode()
    def evaluate_novel_channel_strategies(self, epoch: int, eval_chunks: Optional[List[str]] = None):
        """
        Evaluate and compare different novel channel handling strategies
        
        Args:
            epoch: Current training epoch (used for logging)
            eval_chunks: List of chunks to evaluate on (default: all chunks)
        
        Returns:
            Dict containing comparison of all strategies
        """
        self.logger.info(f"Starting novel channel adaptation evaluation for epoch {epoch}")
        self.model.eval()
        
        # Set up chunks to evaluate
        if eval_chunks is None:
            eval_chunks = self.all_chunks
        ...
        for strategy in strategies:
            self.logger.info(f"Evaluating novel channel strategy: {strategy}")
            
            # Get model to modify (handle DataParallel or DDP)
            if isinstance(self.model, (nn.DataParallel, nn.parallel.DistributedDataParallel)):
                model = self.model.module
            else:
                model = self.model
            
            # Access the patch_embed component that handles novel channels
            patch_embed = model.feature_extractor.patch_embed
            
            # Save original strategy
            original_strategy = patch_embed.novel_channel_strategy
            
            # Set current strategy for evaluation
            patch_embed.novel_channel_strategy = strategy
            
            # If weighted interpolation, also test different interpolation factors
            if strategy == "weighted_interpolation":
                interp_results = {}
                for factor in [0.3, 0.5, 0.7, 0.9]:
                    self.logger.info(f"Testing interpolation factor: {factor}")
                    original_factor = patch_embed.emb_interp_factor
                    patch_embed.emb_interp_factor = factor
                    
                    # Run evaluation
                    strategy_results = self.eval_morphem70k(
                        epoch=epoch, 
                        new_channel_init=None,
                        eval_chunks=eval_chunks
                    )
                    
                    # Store results with factor information
                    interp_results[f"factor_{factor}"] = strategy_results
                    
                    # Reset factor
                    patch_embed.emb_interp_factor = original_factor
                    
                results[strategy] = interp_results
            else:
                # Run standard evaluation with this strategy
                strategy_results = self.eval_morphem70k(
                    epoch=epoch, 
                    new_channel_init=None,
                    eval_chunks=eval_chunks
            ...
            patch_embed.novel_channel_strategy = original_strategy
        ...
        for name, res in results.items():
            if name != "weighted_interpolation":
                if res and "score/final_score/None" in res:
                    score = res["score/final_score/None"]
                    summary[f"strategy_score/{name}"] = score
                    
                    if score > best_score:
                        best_score = score
                        best_strategy = name
            else:
                # Handle weighted_interpolation with different factors
                best_factor = None
                best_factor_score = -float('inf')
                
                for factor_name, factor_res in res.items():
                    if factor_res and "score/final_score/None" in factor_res:
                        factor_score = factor_res["score/final_score/None"]
                        summary[f"strategy_score/weighted_interpolation_{factor_name}"] = factor_score
                        
                        if factor_score > best_factor_score:
                            best_factor_score = factor_score
                            best_factor = factor_name
                            
                        if factor_score > best_score:
                            best_score = factor_score
                            best_strategy = f"weighted_interpolation_{factor_name}"
        
        if best_strategy:
            self.logger.info(f"Best novel channel strategy: {best_strategy} with score {best_score}")
    ...
    def _finish_training(self):
        ...
        self.logger.info("Starting novel channel strategy comparison...")
        self.evaluate_novel_channel_strategies(epoch=self.cfg.train.num_epochs)
