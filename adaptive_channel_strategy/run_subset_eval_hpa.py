...
subset_indices_within_hpa = [4,5,6]  # for example
hpa_mapper = [3,4,5,6]  # current CP
selected_channels = [hpa_mapper.index(i) for i in subset_indices_within_hpa]  # → [0, 4, 3]

if len(selected_channels) != len(subset_indices_within_hpa):
    raise ValueError(f"Some indices in {subset_indices_within_hpa} are not in hpa_mapper {hpa_mapper}")
...
cfg.eval.dest_dir = f"/projectnb/cs598/projects/Modalities_Robustness/adaptive_channel_strategy/results/subset_eval_hpa_{'_'.join(map(str, selected_channels))}"
...
trainer.model.feature_extractor.current_chunk_name = "HPA"
trainer.model.feature_extractor.current_epoch = 60

#trainer.model.feature_extractor.mapper["CP"] = subset_indices_within_cp
trainer.model.feature_extractor.patch_embed.mapper["HPA"] = subset_indices_within_hpa
# === Run evaluation ===
trainer.eval_morphem70k(epoch=60, new_channel_init="",  eval_chunks=["HPA"])
