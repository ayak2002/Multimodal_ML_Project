...
from models.dichavit_adaptive import DiChaViTAdaptive # need 4/9
...
class Trainer:
    def __init__(self, cfg: MyConfig) -> None:
        ...
        if self.cfg.dataset.name in ["morphem70k"]:
            ...
            #3/28
        # elif self.cfg.dataset.name == "morphem70k_v2":
        #     if len(self.cfg.dataset.in_channel_names) == 12:
        #         self.mapper = {"Allen": [0, 1, 2], "HPA": [3, 4, 5, 6], "CP": [7, 8, 9, 10, 11]}
        #     else:
        #         self.mapper = {
        ...
        #     self.mapper = {"HPA": [0, 1, 2, 3]}
            ...
            self.cfg.train.training_chunks = "train"
        # elif hasattr(self.cfg.data_chunk, "chunk"):  # fallback for simple single-chunk configs like CP.yaml
        #     self.mapper = {
        #         self.cfg.data_chunk.chunk: self.cfg.dataset.in_channel_names
        #     }
        #     self.all_chunks = [self.cfg.data_chunk.chunk]
        #     self.cfg.train.training_chunks = self.cfg.data_chunk.chunk
        else:
    ...
    def _forward_model(
        ...
        **kwargs,
        ...
        @return:
        """
        # # Move x to the correct device (helps avoid CPU vs CUDA mismatch errors)
        # x = x.to(self.device)
        # # Ensure kwargs are device-aware too if needed
        # for key in kwargs:
        ...
        if self.forward_mode == "need_chunk_name":
            ...
            output = self.model(x)
            #output = self.model(x, chunk_name=chunk_name)
        return output
    ...
    def eval_subset_channels(self):
        #basically just making it work for v2 4/6
        # dataset_name = self.cfg.dataset.name
        # if dataset_name == "morphem70k_v2":
        #     dataset_name = "morphem70k"
        eval_loader = self.test_loaders[self.cfg.dataset.name]
        #gotta index it properly 4/6
        #print("DEBUG: Available test_loaders keys: ", self.test_loaders.keys())
        #print("DEBUG: Requested dataset_name: ", dataset_name)
        print("DEBUG: self.cfg.data_chunk =", self.cfg.data_chunk)
        print("DEBUG: self.cfg.data_chunk['chunks'] =", self.cfg.data_chunk["chunks"])
        #eval_loader = self.test_loaders[dataset_name]

        # all_acc = {}
        # start_time = time.time()
        # self.model.eval()

        # for chunk_name in ["Allen", "HPA", "CP"]:
        #     if chunk_name not in self.test_loaders:
        #         print(f"Chunk {chunk_name} not in test_loaders, skipping...")
        #         continue
        ...
        if isinstance(eval_loader, dict):
                #if "test" not in eval_loader:  
            if chunk_name not in eval_loader:
                #print(f"'test' not in eval_loader[{chunk_name}], skipping...")
        ...
        #index 4/6
            #channels = self.cfg.data_chunk["chunks"][0][dataset_name]["train"]
            #channels = self.cfg.data_chunk["chunks"][0][self.cfg.dataset.name]["train"]
            #channels = self.cfg.data_chunk["chunks"][0][dataset_name]["train"]
            #channels = self.cfg.data_chunk["chunks"][0][self.cfg.dataset.name]["train"]
            # During the loop over chunk_name...
        all_acc = {}
        start_time = time.time()
        ...
        for n_channels in tqdm(range(start, end, -1)):
            ...
            #             print(f"batch {bid}/{len(eval_loader)}")
            #             batch = utils.move_to_cuda(batch, self.device)
            #             #print("DEBUG: batch keys =", batch.keys() if isinstance(batch, dict) else "not a dict")
            ...
            #                 x = batch["image"]
            #                 y = batch["label"] if "label" in batch else None
            ...
            #                     y=None
            #                 else:
            #                     raise ValueError(f"Unexpected tuple batch: {batch}")
            ...
            #                 raise ValueError(f"Unexpected batch format: {batch}")
                        ...
                        #print("channel_combinations type:", type(channel_combinations))
                        #print("channel_combinations value:", channel_combinations)

                        #x = x[:, selected, :, :].clone()             #commented 4/7
                        #x = x[:, tuple(channel_combinations), :, :].clone()
                        ...
                        # # === LOGGING CONTEXT ===
                        # model.current_epoch = self.cfg.train.num_epochs  # Or pass epoch in args if needed
                        # model.current_chunk_name = chunk_name
                        # model.current_selected_channels = selected
            ...
            #all_acc[f"{chunk_name}_{n_channels}"] = acc_n_channels
            ...
            self.logger.info({f"sub_channels/{chunk_name}/{n_channels}": mean_})
            break
        end_time = time.time()
    ...
    def evaluate_model(self, epoch: int):
        if self.cfg.eval.only_eval_first_and_last:
            if epoch != 0 and epoch != self.cfg.train.num_epochs:
                return None  ## bail out, skip this expensive evaluation
        if self.cfg.dataset.name in ["Allen", "HPA", "CP", "morphem70k", "morphem70k_v2"]:
    ...
    def eval_morphem70k(
        ...
        def log_res(eval_cfg, knn_metric):
            ...
            # eval_cfg.feature_dir = os.path.join(eval_cfg.feature_dir, dataset)
            # commenting above for now 4/9
            # Only append chunk subfolder if evaluating all chunks
            # if eval_chunks is None or len(eval_chunks) == 3:
            #     eval_cfg.feature_dir = os.path.join(eval_cfg.feature_dir, self.cfg.dataset.name)
            # Otherwise, use feature_dir as-is (single chunk eval)
            if eval_chunks is not None and len(eval_chunks) == 1:
                chunk = eval_chunks[0]
                #eval_cfg.feature_dir = os.path.join(eval_cfg.feature_dir)
                eval_cfg.feature_file = "features.npy"
                features_path = os.path.join(eval_cfg.feature_dir, eval_cfg.feature_file)
            else:
                # Default behavior for all-chunk evaluation
                chunk = self.cfg.dataset.name
                #eval_cfg.feature_dir = os.path.join(eval_cfg.feature_dir, chunk)
                if chunk not in eval_cfg.feature_dir:
                    eval_cfg.feature_dir = os.path.join(eval_cfg.feature_dir, chunk)

                features_path = None  # Let `run_benchmark()` determine per chunk
        ...
        # If evaluating only one chunk, save features in that chunk's subdir
        if eval_chunks is not None and len(eval_chunks) == 1:
            eval_cfg.feature_dir = os_join(eval_cfg.feature_dir.format(FOLDER_NAME=FOLDER_NAME), eval_chunks[0])
        else:
            eval_cfg.feature_dir = eval_cfg.feature_dir.format(FOLDER_NAME=FOLDER_NAME)

        utils.mkdir(eval_cfg.feature_dir)
        ...
        for chunk_name in eval_chunks:
            ...
            for bid, batch in tqdm(enumerate(eval_loader), total=total_iterations):
                ...
                # self.model.current_chunk_name = chunk_name
                # self.model.current_selected_channels = (
                #     channel_combinations if channel_combinations is not None
                #     else list(range(x.shape[1]))  # default to all channels
                # )
                # print("DEBUG: x =", x)
            ...
            #commenting above for subset tests

            # Save features directly inside eval_cfg.feature_dir without nested chunk folders
            out_path = os_join(eval_cfg.feature_dir, eval_cfg.feature_file)
            utils.mkdir(os.path.dirname(out_path))  # Create the directory if needed
        ...
        if cosine_metrics and self.cfg.dataset.name in ["morphem70k", "morphem70k_v2"]:
            final_scores = {}
    ...
    def train_one_batch_morphem70k(
        ...
        for chunk_name in all_chunks:
            ...
            with torch.cuda.amp.autocast(enabled=self.use_amp):
                ...
                if self.cfg.train.miro:
                    ...
                    loss = (
                        proxy_loss(self.model.module.proxies if isinstance(self.model, nn.DataParallel) else self.model.proxies, y_pred, y, scale) + extra_loss * self.extra_loss_lambda
                    )
                    ...
                    loss = (
                        proxy_loss(self.model.module.proxies if isinstance(self.model, nn.DataParallel) else self.model.proxies, output, y, scale) + extra_loss * self.extra_loss_lambda
                    )
                    if self.debug:
                        print("loss", loss)
        ...
        loss_dict = {
            ...
            "TRAINING_LOSS_SHUFFLE_ALL/channel_proxy_loss": (
                extra_loss.mean().item() if isinstance(extra_loss, Tensor) else 0.0
            ),
        }  ## loss on training
    ...
    def _load_model(self, path):
        ...
        # if "scheduler_params" in state_dict: commented 4/4
        #     self.scheduler.load_state_dict(state_dict["scheduler_params"])  commented 4/4
        if "scheduler_params" in state_dict and self.scheduler is not None:
