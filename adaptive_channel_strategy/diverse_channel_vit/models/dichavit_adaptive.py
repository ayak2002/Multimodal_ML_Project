...
from save_attention_scores import save_attention_scores
...
print(">>> USING ADAPTIVE dichavit.py")
class PatchEmbedPerChannel(nn.Module):
    ...
    def __init__(
        ...
        print("DEBUG: proxy_loss_lambda =", self.cfg.proxy_loss_lambda) 
        ...
        #new code for mlp 3/25
        if self.use_channelvit_channels:
            self.channel_attention = nn.Sequential(
                nn.LayerNorm(embed_dim),  # Normalize before scoring
                nn.Linear(embed_dim, embed_dim // 2),
                nn.ReLU(),
                nn.Linear(embed_dim // 2, 1),  # One score per channel
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.use_channelvit_channels:
            ...
            #channel_ids = torch.tensor(cur_channels, device=x.device)
            channel_ids = torch.tensor(cur_channels, dtype=torch.long, device=self.channel_embed.weight.device)
            channel_embed = self.channel_embed(channel_ids)
        b, Cin, h, w = x.shape
        Cin_original = Cin
        ...
        x = x.to(self.proj.weight.device) #avantika 4/7
        ...
        #     print("[PatchEmbed] Applying adaptive channel reweighting")

        #     cur_channels = self.mapper[chunk_name]  # Example: [7, 8, 9, 10, 11]
        #     # if hasattr(self, "current_selected_channels"):
        #     #     selected_channels = self.current_selected_channels
        #     # else:
        #     #     selected_channels = list(range(x.shape[1]))  # Default to all
        #     # if hasattr(self, "current_selected_channels") and self.current_selected_channels is not None:
        #     #     active_channel_ids = self.current_selected_channels
        #     #     selected_channels = list(range(len(active_channel_ids)))  # This makes selected_channels safe to use

        #     #     # selected_channels = self.current_selected_channels
        #     #     # if max(selected_channels) >= len(cur_channels):
        #     #     #     raise ValueError(
        #     #     #         f"[PatchEmbed] selected_channels index out of range!\n"
        #     #     #         f"selected_channels: {selected_channels}, cur_channels: {cur_channels}"
        #     #     #     )
        #     # else:
        #     #     #selected_channels = list(range(len(cur_channels)))  # Fallback to valid range
        #     #     #active_channel_ids = self.mapper[chunk_name]
        #     #     selected_channels = list(range(len(cur_channels)))  # Fallback to valid range
        #     #     active_channel_ids = self.mapper[chunk_name]
        #     if hasattr(self, "current_selected_channels") and self.current_selected_channels is not None:
        #         selected_channels = self.current_selected_channels
        #         active_channel_ids = [cur_channels[i] for i in selected_channels]
        #     else:
        #         # Default fallback: use all cur_channels
        #         active_channel_ids = cur_channels


        #     # Subset the actual channels used for this sample
        #     #print(f"[PatchEmbed] cur_channels: {cur_channels}, selected_channels: {selected_channels}")
        ...
        #             f"selected_channels: {selected_channels}, cur_channels: {cur_channels}"
        #         )
        #     print(f"[PatchEmbed] cur_channels: {cur_channels}, active_channel_ids: {active_channel_ids}")
        #     #active_channel_ids = [cur_channels[i] for i in selected_channels]

        #     # Lookup embeddings only for selected channels
        #     channel_embed_subset = self.channel_embed(torch.tensor(active_channel_ids, device=x.device))  # (Cin_sub, Cout)

        #     # channel_embed shape: (Cin, embed_dim)
        #     #channel_embed = repeat(channel_embed, "Cin Cout -> B Cout Cin", B=x.shape[0])  # (B, embed_dim, Cin)
        #     channel_embed = repeat(channel_embed_subset, "Cin Cout -> B Cout Cin", B=x.shape[0])

        #     # Step 1: Transpose for attention scoring: (B, Cin, embed_dim)
        #     channel_embed_perm = channel_embed.permute(0, 2, 1)

        #     # Step 2: Pass through MLP to get (B, Cin, 1) --> squeeze --> (B, Cin)
        #     scores = self.channel_attention(channel_embed_perm).squeeze(-1)

        #     # Step 3: Normalize
        #     scores = torch.softmax(scores, dim=1)  # (B, Cin)

        #     # Step 4: Apply attention weights to channel embeddings
        #     channel_embed_weighted = channel_embed * scores.unsqueeze(1)  # (B, embed_dim, Cin)

        #     # Step 5: Add weighted channel embeddings to patch tokens
        #     x = x[:, :, selected_channels, :, :] #4/8
        #     assert x.shape[2] == channel_embed_weighted.shape[2], f"x Cin: {x.shape[2]}, channel_embed Cin: {channel_embed_weighted.shape[2]}"
        #     x += channel_embed_weighted.unsqueeze(-1).unsqueeze(-1)
        if self.use_channelvit_channels:
            print("[PatchEmbed] Applying adaptive channel reweighting")
            cur_channels = self.mapper[chunk_name]  # e.g., [7, 8, 9, 10, 11]

            # 1. Map global to local channel indices
            global_to_local = {ch: i for i, ch in enumerate(cur_channels)}

            selected_channels = []
            # if hasattr(self, "current_selected_channels") and self.current_selected_channels is not None:
            #     for c in self.current_selected_channels:
            #         if c in global_to_local:
            #             selected_channels.append(global_to_local[c])
            #         else:
            #             print(f"[PatchEmbed] Skipping channel {c} (not in current chunk {chunk_name})")
            #     # try:
            #     #     selected_channels = [global_to_local[c] for c in self.current_selected_channels]
            #     # except KeyError as e:
            #     #     raise ValueError(f"[PatchEmbed] Channel {e} not in current chunk ({chunk_name}): {cur_channels}")
            # else:
            #     selected_channels = list(range(len(cur_channels)))
            if hasattr(self, "current_selected_channels") and self.current_selected_channels is not None:
                try:
                    selected_channels = [global_to_local[c] for c in self.current_selected_channels]
                except KeyError as e:
            ...
            #selected_channels = [global_to_local[c] for c in getattr(self, "current_selected_channels", cur_channels) if c in global_to_local]


            if len(selected_channels) == 0:
                raise ValueError(f"[PatchEmbed] No valid channels selected from {self.current_selected_channels} for chunk {chunk_name}")


            # 2. Select active channels by global ID
            active_channel_ids = [cur_channels[i] for i in selected_channels]

            print(f"[PatchEmbed] cur_channels: {cur_channels}, selected_channels: {selected_channels}, active_channel_ids: {active_channel_ids}")

            # 3. Lookup channel embeddings
            channel_embed_subset = self.channel_embed(torch.tensor(active_channel_ids, device=x.device))
            

            # 4. Apply adaptive reweighting
            channel_embed = repeat(channel_embed_subset, "Cin Cout -> B Cout Cin", B=x.shape[0])
            channel_embed_perm = channel_embed.permute(0, 2, 1)
            scores = self.channel_attention(channel_embed_perm).squeeze(-1)
            scores = torch.softmax(scores, dim=1)
            channel_embed_weighted = channel_embed * scores.unsqueeze(1)

            # 5. Subset x using selected_channels (local indices!)
            #x = x[:, :, selected_channels, :, :]
            assert x.shape[2] == channel_embed_weighted.shape[2], \
                f"x Cin: {x.shape[2]}, channel_embed Cin: {channel_embed_weighted.shape[2]}"
            x += channel_embed_weighted.unsqueeze(-1).unsqueeze(-1)
            ...
            self.last_channel_scores = scores.detach().cpu()

            # (Optional) Save scores for logging/debug
            #self.last_channel_scores = scores.detach().cpu()     #commented 4/8
            # if not self.training:
            #     import csv
            #     import os
            #     out_path = "/projectnb/cs598/projects/Modalities_Robustness/adaptive_channel_strategy/attention_scores_log.csv"
            #     os.makedirs(os.path.dirname(out_path), exist_ok=True)
            #     with open(out_path, "a") as f:
            #         writer = csv.writer(f)
            #         for row in self.last_channel_scores:
            #             writer.writerow(row.tolist())  # each row = channel weights for one image
            # Before saving last_channel_scores, ensure it's valid
            ...
            #     chunk_name = getattr(self, "current_chunk_name", "unknown")
            #     selected_channels = getattr(
            #         self, "current_selected_channels",
            #         list(range(self.last_channel_scores.shape[1]))
            #     )

            #     # Define file path with timestamp or chunk info to avoid overwriting
            #     out_dir = "/projectnb/cs598/projects/Modalities_Robustness/adaptive_channel_strategy/attention_logs"
            #     os.makedirs(out_dir, exist_ok=True)

            #     timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            #     out_path = os.path.join(out_dir, f"attn_epoch{epoch}_{chunk_name}_{timestamp}.csv")
            

            #     # Check if file exists for writing header
            ...
            #         if write_header:
            #             if write_header:
            #                 header = ["epoch", "chunk", "selected_channels"] + \
            #                     [f"channel_{i}" for i in range(self.last_channel_scores.shape[1])]
            ...
            if not self.training:
                epoch = getattr(self, "current_epoch", -1)
                chunk_name = getattr(self, "current_chunk_name", "unknown")
                selected_channels = getattr(self, "current_selected_channels", list(range(self.last_channel_scores.shape[1])))

                if self.last_channel_scores.isnan().any():
                    print("NaN detected in attention scores!")
                    self.last_channel_scores = torch.nan_to_num(self.last_channel_scores, nan=0.0)

                save_attention_scores(
                    scores=self.last_channel_scores,
...
class ChannelVisionTransformer(nn.Module):
    ...
    def forward(
        ...
        #print(">>> Using modified PatchEmbedPerChannel with adaptive channel weights!")
        B, _, w, h = x.shape
...
class DiChaViTAdaptive(nn.Module):
...
def dichavit_adaptive(cfg: Model, **kwargs) -> DiChaViTAdaptive:
    return DiChaViTAdaptive(config=cfg, **kwargs)
