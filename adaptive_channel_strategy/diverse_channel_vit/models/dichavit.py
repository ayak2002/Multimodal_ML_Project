...
print(">>> USING BASELINE dichavit.py")
    ...
    def forward(
        ...
        **kwargs,
        ...
        if hasattr(self, "current_selected_channels") and self.current_selected_channels is not None:
            # Mapping from global channel ID to local index within cur_channels
            print("[Debug] Checking selected channels...")
            assert hasattr(self, "current_selected_channels"), "current_selected_channels is missing!"
            ...
            # Only keep the selected channel IDs
            ...
            print(f"[Subset Eval] Selected channels (local indices): {selected_channels}")
            print(f"[Subset Eval] x shape after slicing: {x.shape}")


            # Get channel embeddings for the selected channels
            ...
            # Add the channel embedding to the patch tokens
            ...
            return x, x.shape[2], 0  # Skipping ortho/proxy loss here

        if self.use_channelvit_channels:
            channel_ids = torch.tensor(cur_channels, dtype=torch.long, device=self.channel_embed.weight.device) #adduing this logic for subset evals 4/10
            channel_embed = self.channel_embed(channel_ids)
            #channel_embed = self.channel_embed(tensor(cur_channels, device=x.device))  #  Cin, embed_dim=Cout     #avantika commented 4/10
        b, Cin, h, w = x.shape
        Cin_original = Cin
        ...
        if self.training and self.enable_sample:
            ...
            if self.cfg.hcs_sampling == "none" or self.cfg.hcs_sampling is None:
                ...
                print("Input shape before projection (should be 3 channels):", x.shape)
                x = x[:, channels_idx, :, :]
                print("[Debug] After slicing: x shape =", x.shape)
                if self.use_channelvit_channels:
        ...
        if self.use_channelvit_channels and (not self.training) and (training_chunks is not None):
            ...
            # ⚠️ Inject subset evaluation logic (for eval-time only)
            if hasattr(self, "current_selected_channels") and self.current_selected_channels is not None:
                global_to_local = {ch: i for i, ch in enumerate(cur_channels)}
                try:
                    selected_channels = [global_to_local[c] for c in self.current_selected_channels]
                ...
                # Subset input tensor and embeddings
                x = x[:, selected_channels, :, :]
                channel_embed = channel_embed[selected_channels]
                cur_channels = [cur_channels[i] for i in selected_channels]  # Update cur_channels too
                print(f"[Eval Subset] Selected channels: {self.current_selected_channels}, x.shape: {x.shape}")
